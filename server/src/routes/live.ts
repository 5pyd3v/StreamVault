...
function publicShape(c: LiveChannelRow) {
  ...
  return {
    ...
    category: c.category,
    posterUrl: toUrl(c.poster_path),
    status: c.status === 'error' ? 'offline' : c.status,
    liveStartedAt: c.live_started_at,
    hlsUrl: c.status === 'live' ? `/uploads/hls/live/${id}/master.m3u8` : '',
  };
}

/**
 * Admin-facing shape. The snake_case row is re-keyed to exactly the camelCase
 * field set the old Mongoose `.toObject()` produced, so the frontend sees no
 * difference. The stream key is never included -- the routes that reveal it add
 * it explicitly (mirroring the old schema-level `select: false`).
 */
function adminShape(c: LiveChannelRow) {
  return {
    _id: String(c.id),
    name: c.name,
    slug: c.slug,
...
router.get('/channels/:slug', protect, async (req: AuthRequest, res) => {
  try {
    const channel = await findChannelBySlug(req.params.slug, true);
...
// by default — everywhere else it's omitted by the key-free column list.
adminRouter.post('/', async (req: AuthRequest, res) => {
  try {
    ...
    res.status(201).json({
      ...adminShape(channel),
      streamKey,
      rtmpUrl: rtmpUrlFor(channel.rtmp_app, streamKey),
    });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
// List all channels (streamKey omitted by the key-free column list)
adminRouter.get('/', async (_req: AuthRequest, res) => {
  try {
    const channels = await listAllChannels();
    res.json(channels.map(c => ({
      ...adminShape(c),
      // The join supplies the owner's name/email, matching the old
      // .populate('owner', 'name email').
      owner: { _id: String(c.owner_id), name: c.owner_name, email: c.owner_email },
      hlsUrl: c.status === 'live' ? `/uploads/hls/live/${String(c.id)}/master.m3u8` : '',
    })));
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
adminRouter.get('/:id/reveal', async (req: AuthRequest, res) => {
  try {
    const channel = await findChannelByIdWithKey(req.params.id);
    if (!channel) return res.status(404).json({ error: 'Channel not found' });
    res.json({
      streamKey: channel.stream_key,
      rtmpUrl: rtmpUrlFor(channel.rtmp_app, channel.stream_key!),
    });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
adminRouter.patch('/:id', async (req: AuthRequest, res) => {
  try {
    ...
    if (recordEnabled !== undefined) patch.recordEnabled = !!recordEnabled;
    if (isEnabled !== undefined) patch.isEnabled = !!isEnabled;

    const channel = await updateChannel(req.params.id, patch);
    if (!channel) return res.status(404).json({ error: 'Channel not found' });

    // Disabling a channel mid-broadcast should actually cut it off
    if (isEnabled !== undefined && !channel.is_enabled) stopChannelStream(req.params.id);

    res.json(adminShape(channel));
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});
...
adminRouter.post('/:id/poster', posterUpload.single('poster'), async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const previousPath = channel.poster_path;
    await setChannelPoster(req.params.id, uploaded.path);
    channel.poster_path = uploaded.path;

    // Drop the file we just replaced (admin-uploaded or auto-captured) so old
    // posters don't pile up in uploads/posters.
    if (previousPath && previousPath !== channel.poster_path) removePosterFile(previousPath);
...
adminRouter.post('/:id/regenerate-key', async (req: AuthRequest, res) => {
  try {
    ...
    // Resets exactly the fields the old save() did: status, currentSessionId, liveHlsPath.
    await setChannelOffline(req.params.id);
...
// Delete the channel. Replay Video rows created from past broadcasts are left alone.
